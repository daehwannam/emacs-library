...
(defun dhnam/get-current-compile-error-file-path ()
  "Return the file path of the next compilation error without opening the file."

  (interactive)
  (let* ((msg (compilation-next-error 0))   ;; get the information of error from the current line
         (loc (compilation--message->loc msg))
         (file (caar (compilation--loc->file-struct loc))))
    (when (called-interactively-p 'interactive)
      (if file
          (message "%s" file)
        (message "No file for this error.")))
    file))
